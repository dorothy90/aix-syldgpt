...
def ensure_opensearch_index():
    ...
    if not opensearch_client.indices.exists(index=OPENSEARCH_INDEX):
        # 임베딩 차원 수 가져오기
        test_embedding = text_embedder.embed_query("test")
        ...
        print(f"✓ OpenSearch 인덱스 생성: {OPENSEARCH_INDEX} (차원: {dimension})")
    ...
    try:
        ...
        for idx, doc in enumerate(documents, 1):
            ...
            result = opensearch_client.index(
                index=OPENSEARCH_INDEX, body=opensearch_doc, refresh=True
            )
            print(f"  ✓ 문서 {idx}/{len(documents)} 저장 완료 (ID: {result['_id']})")
...
if __name__ == "__main__":
    # 인덱스 초기화 (선택사항 - 주의: 모든 데이터 삭제됨)
    # if opensearch_client.indices.exists(index=OPENSEARCH_INDEX):
    #     opensearch_client.indices.delete(index=OPENSEARCH_INDEX)
    #     print(f"기존 인덱스 삭제: {OPENSEARCH_INDEX}")

    # 여러 문서 처리
    files_to_process = [
        # "/Users/daehwankim/Documents/langgraph-tutorial-main/RAG_CHATBOT_OS/files/example.pptx",
        "/Users/daehwankim/Documents/langgraph-tutorial-main/RAG_CHATBOT_OS/files/people.json",
        "/Users/daehwankim/Documents/langgraph-tutorial-main/RAG_CHATBOT_OS/files/sample-word-document.docx",
        # "/Users/daehwankim/Documents/langgraph-tutorial-main/RAG_CHATBOT_OS/files/titanic.xlsx",
    ...
    print(f"\n{'='*60}")
    print(f"전체 처리 완료! 총 {total_docs}개 문서가 OpenSearch에 저장되었습니다.")
    print(f"{'='*60}")
