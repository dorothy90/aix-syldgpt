...
# WADS CSV 파일 경로 설정
_WADS_CSV_PATH = (
    Path(__file__).parent.parent.parent / "wads_agent_html" / "wads_agent_html.csv"
)
...
def _load_wads_data() -> pd.DataFrame:
    """WADS CSV 데이터 로드"""

    df = pd.read_csv(_WADS_CSV_PATH)
    return df


def _filter_wads_data(
    df: pd.DataFrame,
    ...
    """선택적 필터 적용"""
    if lotcd:
        df = df[df["lotcd"].str.contains(lotcd, case=False, na=False)]
    if end_tm:
        df = df[df["end_tm"].str.contains(end_tm, case=False, na=False)]
    if ctn_desc:
        df = df[df["ctn_desc"].str.contains(ctn_desc, case=False, na=False)]
    return df
...
def wads_query_data(
    ...
    if df.empty:
        _TOOL_PAYLOAD_STORAGE["query"] = [
            {
                "error": "NoData",
                "detail": "WADS CSV 파일을 찾을 수 없거나 비어있습니다.",
            }
        ]
        return "오류: WADS CSV 파일을 찾을 수 없거나 비어있습니다."

    filtered_df = _filter_wads_data(df, lotcd, end_tm, ctn_desc)
